...
<script>
  ...
  export default {
    ...
    computed: {
      showEndDate: {
        get () {
          return this.contract.Linkage != null && (this.contract.Linkage === 2 || this.contract.Linkage === 3 || this.contract.Linkage === 13)
        }
      },
      possitionWath: {
        get () {
    ...
    methods: {
      ...
      put () {
        if (this.canEdit) {
          ...
          if (this.valid()) {
            this.contract.PeopleId = this.PeopleId
            axios.put('/Contract/' + this.ContractId, this.contract, {
              headers: {
                'token': localStorage.getItem('token')
              }
            })
