  ...
  <div class="row-UO">
    <div class="card-UO">
      <div class="search-bar-UO">
        <input v-model="searchTerm" @input="handleSearch" placeholder="Buscar por CUNI, documento, nombre, dependencia" class="search-input-UO" />
  ...
  export default {
    data () {
      return {
        searchTerm: '',
        tableColumns: [
          { label: 'CUNI', prop: 'CUNI' },
          { label: 'Documento', prop: 'Document' },
          { label: 'Nombre Completo', prop: 'FullName' },
          { label: 'Dependencia', prop: 'Dependency' },
          { label: 'Cod. Dependencia', prop: 'DependencyCod' },
          { label: 'BranchesId', prop: 'BranchName' },
          { label: 'Sede', prop: 'BranchesId' }
        ],
        filteredData: [],
        currentPage: 1,
    ...
    methods: {
      shouldShowColumn (column) {
        ...
        return column && column.prop && column.prop !== 'BranchName'
      },
      async generateExcel () {
        try {
          ...
          if (this.filteredData.length > 0) {
            ...
            const wsSearch = XLSX.utils.aoa_to_sheet([['CUNI', 'Documento', 'Nombre Completo', 'Dependencia', 'Código Dependencia', 'Sede']])
            const excludedColumnsIndices = [5]
            ...
            XLSX.writeFile(wbSearch, 'PersonasSAP_Busqueda.xlsx')
          } else {
            // Generar Excel de toda la respuesta
            const wbAll = XLSX.utils.book_new()
            const wsAll = XLSX.utils.aoa_to_sheet([['CUNI', 'Documento', 'Nombre Completo', 'Dependencia', 'Código Dependencia', 'Sede']])
            const excludedColumnsIndices = [5]
            ...
            XLSX.writeFile(wbAll, 'PersonasSAP_Completa.xlsx')
          }
        } catch (error) {
      ...
      async generatePDF () {
        try {
          ...
          if (this.filteredData.length > 0) {
            ...
            pdfSearch.save('PersonasSAP_Busqueda.pdf')
          } else {
            ...
            pdfAll.save('PersonasSAP_Completa.pdf')
          }
        } catch (error) {
          console.error('Error al obtener datos:', error)
        }
