<template>
  <div class="col-md-12">
    <div  class="uploader">
      <div class="sa" v-if="this.$store.state.civ.uploadedFiles.array.includes(fileType)">
        ...
        <button v-if="this.$store.state.civ.uploadedFiles.state==='INICIADO'"  type="button" class="btn btn-wd btn-danger" style="margin-top: 25px; margin-left: 10px" @click="deleteFile">
...
<script>
  ...
  export default {
    ...
    computed: {
      valid: {
        get () {
          return this.$store.state.civ.uploadedFiles.array.includes(this.fileType)
        }
      }
    },
    watch: {
      valid (newValue, oldValue) {
        this.$emit('uploaded', newValue)
      }
    },
    methods: {
      deleteFile () {
        axios.delete(this.url, {
          data: {
            BranchesId: this.$store.state.civ.BranchesId,
            FileType: this.$store.state.civ.FileType
          }
        }).then(response => {
      ...
      submitFile () {
        ...
        .then(response => {
          ...
          this.$store.dispatch('civ/uploadedFiles').then(r => {
