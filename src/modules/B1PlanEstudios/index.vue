  ...
  <div class="row-UO">
    <div class="card-UO">
      <div class="search-bar-UO">
        <input v-model="searchTerm" @input="handleSearch" placeholder="Buscar por código, descripción o unidad organizacional" class="search-input-UO" />
  ...
  export default {
    data () {
      return {
        ...
        tableColumns: [
          ...
          { label: 'Descripción', prop: 'PrcName' },
          { label: 'Válido Desde', prop: 'ValidFrom' },
          { label: 'Válido Hasta', prop: 'ValidTo' },
          { label: 'Código Dep', prop: 'U_CODIGO_DEPARTAMENTO' },
          { label: 'Código Seg', prop: 'U_CODIGO_SEGMENTO' },
          { label: 'Unidad Organizacional', prop: 'UO' },
          { label: 'Num Int Car', prop: 'U_NUM_INT_CAR' },
          { label: 'Branches', prop: 'BranchesId' }
        ],
        filteredData: [],
        currentPage: 1,
    ...
    methods: {
      shouldShowColumn (column) {
        ...
        return column && column.prop && column.prop !== 'U_CODIGO_DEPARTAMENTO' && column.prop !== 'U_CODIGO_SEGMENTO' && column.prop !== 'BranchesId' && column.prop !== 'U_NUM_INT_CAR'
      },
      async generateExcel () {
        try {
          ...
          if (this.filteredData.length > 0) {
            ...
            const wsSearch = XLSX.utils.aoa_to_sheet([['Código', 'Descripción', 'Unidad Organizacional']])
            ...
            XLSX.writeFile(wbSearch, 'Programa_Académico_Busqueda.xlsx')
          } else {
            // Generar Excel de toda la respuesta
            const wbAll = XLSX.utils.book_new()
            const wsAll = XLSX.utils.aoa_to_sheet([['Código', 'Descripción', 'Unidad Organizacional']])
            const excludedColumnsIndices = [2, 3, 4, 5, 7, 8]
            ...
            XLSX.writeFile(wbAll, 'Programa_Académico_Completa.xlsx')
          }
        } catch (error) {
      ...
      async generatePDF () {
        try {
          ...
          if (this.filteredData.length > 0) {
            ...
            pdfSearch.save('Programa_Académico_Busqueda.pdf')
          } else {
            ...
            pdfAll.save('Programa_Académico_Completa.pdf')
          }
        } catch (error) {
          console.error('Error al obtener datos:', error)
        }
      ...
      generatePDFContent (pdf, data) {
        ...
        pdf.setFontSize(18)
        pdf.text('Información Programa Académico', 145, 25, null, null, 'center')
        // Agrega la información de la tabla al PDF
        pdf.setFontSize(14)
        pdf.text('Universidad Católica Boliviana "San Pablo"', 20, 10)
        const headers = ['Código', 'Descripción', 'Válido Desde', 'Válido Hasta', 'Código Dep', 'Código Seg', 'Unidad Organizacional', 'Num Int Car', 'Branches']
      ...
      async getDataFromURL () {
        try {
          ...
          console.log('Obtengo Programa Académico', response.data)
