  ...
  <div class="row-UO">
    <div class="card-UO">
      <div class="search-bar-UO">
        <input v-model="searchTerm" @input="handleSearch" placeholder="Buscar por código, nombre, NIT o ciudad" class="search-input-UO" />
  ...
  export default {
    data () {
      return {
        searchTerm: '',
        tableColumns: [
          { label: 'Código', prop: 'CardCode' },
          { label: 'Nombre', prop: 'CardName' },
          { label: 'NIT', prop: 'LicTradNum' },
          { label: 'Tipo', prop: 'CardType' },
    ...
    methods: {
      shouldShowColumn (column) {
        ...
        return column && column.prop && column.prop !== 'Series' && column.prop !== 'GroupCode' && column.prop !== 'Currency' && column.prop !== 'Country'
      },
      async generateExcel () {
        try {
          ...
          if (this.filteredData.length > 0) {
            ...
            const wsSearch = XLSX.utils.aoa_to_sheet([['Código', 'Nombre', 'NIT', 'Tipo', 'Ciudad']])
            const excludedColumnsIndices = [4, 5, 6, 8]
            ...
            XLSX.writeFile(wbSearch, 'SocioDeNegocio_Busqueda.xlsx')
          } else {
            // Generar Excel de toda la respuesta
            const wbAll = XLSX.utils.book_new()
            const wsAll = XLSX.utils.aoa_to_sheet([['Código', 'Nombre', 'NIT', 'Tipo', 'Ciudad']])
            const excludedColumnsIndices = [4, 5, 6, 8]
            ...
            XLSX.writeFile(wbAll, 'SocioDeNegocio_Completa.xlsx')
          }
        } catch (error) {
      ...
      async generatePDF () {
        try {
          ...
          if (this.filteredData.length > 0) {
            ...
            pdfSearch.save('SocioDeNegocio_Busqueda.pdf')
          } else {
            ...
            pdfAll.save('SocioDeNegocio_Completa.pdf')
          }
        } catch (error) {
          console.error('Error al obtener datos:', error)
        }
      ...
      async getDataFromURL () {
        try {
          ...
          console.log('Esto obtengo en BP: ', response.data)
