<template>
  <button class="px-0 m-0 btn btn-info" @click="generatePDF">Generar PDF</button>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      generatePDF () {
        ...
        doc.setFontSize(15)
        doc.text('Actividades Académicas en Pregrado ' + this.type, 85, 32)
        ...
        for (let i = 0; i < this.careers.length; i++) {
          ...
          for (var tableIndex = index; tableIndex < index + tableLength; tableIndex++) {
            ...
            tableBody.push([files[tableIndex].TeacherFullName, files[tableIndex].Origen, files[tableIndex].TipoTarea, files[tableIndex].Modulo, files[tableIndex].NumeroContrato, files[tableIndex].Horas, files[tableIndex].MontoHora, files[tableIndex].Total_Bruto, files[tableIndex].Deduccion, files[tableIndex].IUE, files[tableIndex].IT, files[tableIndex].IUEExt, files[tableIndex].Total_Neto, files[tableIndex].Observaciones, files[tableIndex].Dup])
          }
          ...
          doc.autoTable({
            ...
            head: [['Docente', 'Origen', 'Tarea', 'Modulo', 'N° Contrato', 'Horas', 'Costo Hora', 'Total Bruto', 'Deduccion', 'RCIVA', 'IT', 'IUEExt', 'Total Neto', 'Observaciones', 'Dup']],
            body: tableBody,
            theme: 'grid',
        ...
        doc.save('ReporteAgrupadoTotalPostgrado.pdf')
        return doc
      },
      loadCareerBody () {
        // Cargar el cuerpo, es decir datos por carrera
        let rec = this.records
        axios.get('PDFReportBodyPostgrado?part=Body;' + this.state + ';' + this.origin)
          .then(response => {
      ...
      loadCareerTotals () {
        ...
        axios.get('PDFReportBodyPostgrado?part=Results;' + this.state + ';' + this.origin)
          .then(response => {
      ...
      loadAllTotals () {
        ...
        axios.get('PDFReportBodyPostgrado?part=FinalResult;' + this.state + ';' + this.origin)
          .then(response => {
            response.data.forEach(function (element) {
