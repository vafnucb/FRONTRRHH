  ...
  export default {
    data () {
      return {
        searchTerm: '',
        tableColumns: [
          { label: 'Código', prop: 'PrjCode' },
          { label: 'Descripción', prop: 'PrjName' },
          { label: 'Valido Hasta', prop: 'ValidTo' },
          { label: 'Activo', prop: 'Active' },
          { label: 'Modalidad', prop: 'U_ModalidadProy' },
          { label: 'Sede', prop: 'U_Sucursal' },
          { label: 'Tipo', prop: 'U_Tipo' },
          { label: 'Unidad Organizacional', prop: 'U_UORGANIZA' },
          { label: 'PEI/PO', prop: 'U_PEI_PO' }
        ],
        filteredData: [],
        currentPage: 1,
    ...
    methods: {
      async generateExcel () {
        try {
          ...
          if (this.filteredData.length > 0) {
            ...
            const wsSearch = XLSX.utils.aoa_to_sheet([['Código', 'Descripción', 'Activo', 'Modalidad', 'Sede', 'Tipo', 'Unidad Organizacional', 'PEI/PO']])
            const excludedColumnsIndices = [2]
            ...
            XLSX.writeFile(wbSearch, 'Proyectos_Busqueda.xlsx')
          } else {
            // Generar Excel de toda la respuesta
            const wbAll = XLSX.utils.book_new()
            const wsAll = XLSX.utils.aoa_to_sheet([['Código', 'Descripción', 'Válido Hasta', 'Activo', 'Modalidad', 'Sede', 'Tipo', 'Unidad Organizacional', 'PEI/PO']])
            const excludedColumnsIndices = [2]
            ...
            XLSX.writeFile(wbAll, 'Proyectos_Completa.xlsx')
          }
        } catch (error) {
      ...
      async generatePDF () {
        try {
          ...
          if (this.filteredData.length > 0) {
            ...
            pdfSearch.save('Proyectos_Busqueda.pdf')
          } else {
            ...
            pdfAll.save('Proyectos_Completa.pdf')
          }
        } catch (error) {
          console.error('Error al obtener datos:', error)
        }
      ...
      generatePDFContent (pdf, data) {
        ...
        const headers = ['Código', 'Descripción', 'Válido Hasta', 'Activo', 'Modalidad', 'Sede', 'Tipo', 'Unidad Organizacional', 'PEI/PO']
        const excludedColumnsIndices = [2] // Índices de las columnas a excluir
