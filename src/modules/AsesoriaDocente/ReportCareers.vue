  ...
  export default {
    ...
    methods: {
      ...
      generatePDF () {
        ...
        doc.setFontSize(15)
        doc.text('Actividades Académicas en Pregrado ' + this.carrera, 85, 32)
        ...
        doc.save('ReporteAgrupadoCarrera.pdf')
        return doc
      },
      loadCareerBody () {
        // Cargar el cuerpo, es decir datos por carrera
        let rec = this.records
        console.log('PDFReportBodyXCarrera?part=Body;' + this.state + ';' + this.origin + ';' + this.carrera)
        axios.get('PDFReportBodyXCarrera?part=Body;' + this.state + ';' + this.origin + ';' + this.carrera)
          .then(response => {
      ...
      loadCareerTotals () {
        ...
        axios.get('PDFReportBodyXCarrera?part=Results;' + this.state + ';' + this.origin + ';' + this.carrera)
          .then(response => {
      ...
      loadAllTotals () {
        ...
        axios.get('PDFReportBodyXCarrera?part=FinalResult;' + this.state + ';' + this.origin + ';' + this.carrera)
          .then(response => {
            response.data.forEach(function (element) {
