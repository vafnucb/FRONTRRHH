  ...
  <div class="row">
    ...
    <div v-if="actions==='LIST'" class="row">
      <div v-if="allIds.length>0">
        ...
        <!--
        <div class="col-md-2" v-if="origen==='OR'">
          <button class="btn btn-info btn-wd" @click="ToOR">Generar archivo Otras Regionales SALOMON</button>
        </div>-->
      </div>
    </div>
    ...
    <template v-if="actions==='ASIGNARFACTURA'">
      ...
      <div class="row">
        <div class="col-md-3 el-col-md-offset-1 form-group">
          <label>NIT</label>
  ...
  export default {
    ...
    data () {
      return {
        ...
        selectBranches: [],
        // datos de la tabla
        headers: ['Docente', 'Categ', 'Modal', 'Tarea', 'Alumno', 'Acta', 'Fecha Acta', 'Horas', 'Costo Hora', 'Total Bruto', 'Total Neto', 'Observaciones', 'Dup'],
        careers: [],
        teachers: [],
        records: [],
        recordsD: [],
        ...
        finalResultD: [],
        // Nuevo PDF x carrera
        headersPDF: ['Docente', 'Categ', 'Modal', 'Tarea', 'Alumno', 'Acta', 'Fecha Acta', 'Horas', 'Costo Hora', 'Total Bruto', 'Total Neto', 'Observaciones', 'Dup'],
        careersPDF: [],
        teachersPDF: [],
        recordsPDF: [],
        recordsDPDF: [],
    ...
    methods: {
      showDateForHistoricOR () {
        ...
        if (vm.SelectedIds.length < 1) {
          swal({
            title: 'Debe seleccionar al menos un registro antes de enviar a historico.',
          ...
          if (vm.SelectedIds.length > 200) {
            swal({
              title: 'No se puede enviar más de 200 registros a historico.',
      ...
      buscarFactura () {
        ...
        axios.get('BuscarFacturaCarrera', {
          ...
          .then(function (response) {
            ...
            if (response.data && response.data.Found) {
              ...
              swal({
                title: 'Factura no encontrada en SIAT',
                text: '¿Es una factura manual?',
                type: 'question',
                showCancelButton: true,
      ...
      send () {
        if (!this.valid() && this.origen === 'OR') {
          console.log('el valor de OR SALOMON es:' + this.valid())
          this.UpdateStateOR()
        } else {
          this.SendHistoric()
        }
      ...
      UpdateStateOR () {
        if (this.origen === 'OR') {
          ...
          console.log('ToPreAprobacionOR?myArray=' + vm.SelectedIds.join() + '&data=' + data)
          swal({
            title: 'Enviar a Historico',
            text: '¿Esta seguro?',
            type: 'warning',
            showCancelButton: true,
            confirmButtonText: 'Si, enviar!',
            ...
            axios.put('ToPreAprobacionOR?myArray=' + vm.SelectedIds.join() + '&data=' + data)
              .then(response => {
                swal({
                  title: 'Buen trabajo!!',
                  text: 'Se enviaron los registros a aprobación',
      ...
      SendHistoric () {
        var vm = this
        var data = this.segmento + ';' + this.mes + ';' + this.gestion
        console.log('SendHistoric?myArray=' + vm.SelectedIds.join() + '&data=' + data)
        swal({
          ...
          axios.put('SendHistoric?myArray=' + vm.SelectedIds.join() + '&data=' + data)
            .then(response => {
              swal({
                title: 'Buen trabajo!!',
                text: 'Se enviaron los registros a aprobación',
      ...
      loadTutoriaIds () {
        let id = this.allIds
        console.log('AsesoriaDocente?by=' + this.estado + '-' + this.origen)
      ...
      loadCareerBody () {
        ...
        console.log('PDFReportBody?part=Body;' + this.estado + ';' + this.origen)
